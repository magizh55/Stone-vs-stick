...
(function () {
  ...
  btnToss.addEventListener('click', () => {
    ...
    /* Decide winner BEFORE spinning so we land on the correct face */
    const winner = Math.random() < 0.5 ? 1 : 2;
    S.cur = winner;

    /* Set both faces — front=Stone, back=Stick */
    coinFront.textContent      = '🪨';
    coinFrontLabel.textContent = 'STONE';
    coinBack.textContent       = '🪵';
    coinBackLabel.textContent  = 'STICK';

    /* Reset coin completely before animating */
    coin.classList.remove('spin-to-front', 'spin-to-back');
    coin.style.transform = 'rotateY(0deg)';
    void coin.offsetWidth; /* force reflow so reset takes effect */

    tossResult.classList.add('hidden');
    coinOverlay.classList.remove('hidden');

    /* Add correct spin class — lands on matching face */
    coin.classList.add(winner === 1 ? 'spin-to-front' : 'spin-to-back');

    /* Show result text after spin finishes (1.2s) */
    setTimeout(() => {
